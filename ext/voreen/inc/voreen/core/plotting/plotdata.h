...
public:
     ...
     *          the same value/tag in colum with index \a groupColumn will be combined in one new PlotRow and inserted
     ...
     *          a colum: the first part defines the source column of this, the second part the source column of
     *          \a otherPlotData.
    ...
    /**
     * \brief   returns the interval from 0 to the maximal sum of absolut values in one row using the columns \a column
     ...
     *                        PlotPickingManager the value -1 encodes wohle row/column.
