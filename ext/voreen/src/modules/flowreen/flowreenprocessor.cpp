...
bool FlowreenProcessor::initialized_(false);
...
tgt::vec3 FlowreenProcessor::mapToFlowBoundingBox(const tgt::vec3& vertex) const {
    const tgt::vec3 dim(static_cast<tgt::vec3>(flowDimensions_ - tgt::ivec3(1)));
...
void FlowreenProcessor::renderBoundingBox(const tgt::vec4& color) const {
    tgt::vec3 urf = mapToFlowBoundingBox(static_cast<tgt::vec3>(flowDimensions_ - tgt::ivec3(1)));
...
void FlowreenProcessor::init() {
    ...
    ShdrMgr.addPath(VoreenApplication::app()->getModulePath("flowreen/glsl"));
