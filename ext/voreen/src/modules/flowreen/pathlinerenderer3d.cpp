...
void PathlineRenderer3D::initialize() throw (VoreenException) {
    ...
    switch (thresholding_) {
        ...
        case THRESHOLDING_INTENSITY:
            if (intensityMasks_.empty() == false) {
                ...
                bool keep = false;
                for (size_t i = 0; i < intensityMasks_.size(); ++i) {
                    const bool& value = intensityMasks_[i]->voxel(r0);
                    if (value == true) {
                        keep = true;
                        break;
                    }
...
size_t PathlineRenderer3D::markContextData(const tgt::vec2& thresholds) {
    ...
    if ((contextCollection_ == 0) || (flowDimensions_ == tgt::ivec3::zero))
        return 0;

    VolumeOperatorIntensityMask mask(thresholds);
    for (size_t i = 0; i < contextCollection_->size(); ++i) {
        if (contextCollection_->at(i) == 0)
            continue;

        Volume* volume = contextCollection_->at(i)->getVolume();
        if ((volume != 0) && (volume->getDimensions() == flowDimensions_))
            intensityMasks_.push_back(mask.apply<VolumeAtomic<bool>*>(volume));
    }
...
size_t PathlineRenderer3D::checkFlows(const bool forceUpdate) {
    ...
    for (size_t i = 0; i < flowCollection_->size(); ++i) {
        if (flowCollection_->at(i) != 0) {
            VolumeFlow3D* flowVolume = dynamic_cast<VolumeFlow3D*>(flowCollection_->at(i)->getVolume());
            if (flowVolume == 0)
                continue;

            if (flowDimensions_ == tgt::ivec3::zero) {
...
{
    tgt::ivec3 grid = ((flowDimensions_ - tgt::ivec3(1)) / static_cast<int>(spacing)) + tgt::ivec3(1);
...
void PathlineRenderer3D::adjustTimestepProperty() {
    VolumeCollection* collection = inportFlows_.getData();
...
void PathlineRenderer3D::renderAsTubes(const tgt::vec4& lineColor) {
    ...
    for (size_t i = 0; i < numPathlines_; ++i) {
        ...
        if (forwardDirection == true) {
            ...
            int aux = static_cast<int>(pathline.size() - 1) - mod;
