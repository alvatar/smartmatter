...
StreamlineRenderer3D::~StreamlineRenderer3D() {
    if (displayLists_ != 0)
        glDeleteLists(displayLists_, numStreamlines_);
...
void StreamlineRenderer3D::process() {
    ...
    // check whether the current VolumeHandle belongs to flow data
    //
    VolumeFlow3D* volFlow = dynamic_cast<VolumeFlow3D*>(currentVolumeHandle_->getVolume());
    if (volFlow == 0) {
    ...
    const VolumeTexture* const tex = currentVolumeHandle_->getVolumeGL()->getTexture();
...
void StreamlineRenderer3D::buildDisplayLists(const Flow3D& flow, const tgt::vec2& thresholds) {
    ...
    displayLists_ = glGenLists(numStreamlines_);
    GLuint i = 0;
    size_t numTries = 0;
