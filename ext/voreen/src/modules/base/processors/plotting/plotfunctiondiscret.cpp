...
std::string PlotFunctionDiscret::getProcessorInfo() const {
    return std::string("interpolate the Data on the PlotPort.PlotFunction.inPort.");
}

void PlotFunctionDiscret::initialize() throw (VoreenException) {
...
void PlotFunctionDiscret::deinitialize() throw (VoreenException) {
...
void PlotFunctionDiscret::readData() {
    ...
    if (dynamic_cast<PlotFunction*>(inPort_.getData())) {
        pData_ = dynamic_cast<PlotFunction*>(inPort_.getData());
    }
    else {
...
void PlotFunctionDiscret::choiceChange() {
    ...
    int length = resetList_.interval.size();
    if (value <= length) {
