...
std::string ExplosionProxyGeometry::getProcessorInfo() const {
    return "Provides a ray casting proxy geometry that can be sectioned into bricks for exploded views."
        "The loop port has to be connected to a ExplosionCompositor."

        "<p>Three bricking modes are offered:"

        "<p>Regular Grid: <br>In this mode the proxy geometry of the volume can be split into a regular "
        "grid by choosing the number of bricks on each axis. <br>"
        "The bricks can \"explode\" by using the gap property of this mode.</p>"

        "<p>Manual Grid: <br>In this mode the proxy geometry of the volume can be split into a manual grid "
        "by manually adding axis-aligned planes as boundaries for the bricks. <br>"
        "As in regularGrid mode bricks can \"explode\" by using the gap property of this mode.</p>"

        "<p>Custom: <br>In addition to manualGrid mode one or several bricks can be selected in this mode, "
        "so that added axis-aligned planes only split the selected bricks and not the whole volume. <br>"
        "The \"explosion\" of the bricks no longer depends on a gap-value. Instead each brick can be translated "
        "everywhere by using the translation property or the drag-and-drop translation mouse event of this mode. <br>"
        "Furthermore, bricks can be hidden in this mode.</p>"

        "The created proxy geometries of regular grid or manual grid mode can be redefined in custom mode by "
        "clicking \"Use this for custom mode\".</p>"

        "<p><b>See</b>: ExplosionCompositor, MeshEntryExitPoints</p>";
}
...
void ExplosionProxyGeometry::deinitialize() throw (VoreenException) {
...
void ExplosionProxyGeometry::process() {
    tgtAssert(inportVolume_.getData()->getVolume(), "no volume");

    int iteration = loopInport_.getLoopIteration();
    tgt::ivec3 numSlices = inportVolume_.getData()->getVolume()->getDimensions();

    /////////////////////////////Bricking process beginning/////////////////////////////////////
    proxyGeometry_->clear();
    renderGeometry_->clear();
    ...
    if (brickingMode_.get() == "regularGrid") {
        ...
        proxyGeometry_->addMesh(MeshGeometry::createCube(
        ...
        if (showBoundingBoxes_.get() == "all" || showBoundingBoxes_.get() == "selected") {
            renderGeometry_->addMesh(MeshGeometry::createCube(
                outputBricklist_.at(iteration).coordLlf + outputBricklist_.at(iteration).translation,
                outputBricklist_.at(iteration).coordUrb + outputBricklist_.at(iteration).translation,
                outputBricklist_.at(iteration).texLlf,
                outputBricklist_.at(iteration).texUrb,
    ...
    outportProxyGeometry_.setData(proxyGeometry_);
    outportRenderGeometry_.setData(renderGeometry_);
    LGL_ERROR;
}
...
void ExplosionProxyGeometry::doBricking() {
    ...
    Volume* volume = inportVolume_.getData()->getVolume();
    ...
    if (brickingMode_.isSelected("regularGrid")) {
        ...
        createRegularOrManualBricklist(numBricksXm_ * numBricksYm_ * numBricksZm_);
        loopInport_.setNumLoopIterations(numBricksXm_ * numBricksYm_ * numBricksZm_);
    }
    else{
        if (customBricklist_.empty())
            createCustomBricklist(' ', 0, 0);

        loopInport_.setNumLoopIterations(customBricklist_.size());
    }
}
...
void ExplosionProxyGeometry::takeForCustom() {
    ...
    for (i = 0; i < customBricklist_.size(); ++i) {
        customBricklist_.at(i).brickId = i;
...
void ExplosionProxyGeometry::createRegularOrManualBricklist(int numIterations) {
    ...
    if (brickingMode_.get() == "regularGrid") {
        ...
        for (it = 0; it < numIterations; ++it) {
            xOffset = it / (numBricksYm_ * numBricksZm_);
            yOffset = (it / numBricksZm_) % numBricksYm_;
...
tgt::vec3 ExplosionProxyGeometry::getSliceWidth(Volume* volume) {
    tgt::vec3 sliceWidth;
...
void ExplosionProxyGeometry::registerForSelecting(std::vector<Brick> bricklist) {
    ...
    for (i = 0; i < bricklist.size(); ++i) {
        tgt::col3 color = idManager_.getColorFromObject((void *)(i + 1));
...
void ExplosionProxyGeometry::onSelectingOneBrickEvent(tgt::MouseEvent * e) {
    ...
    void* brick = idManager_.getObjectAtPos(pos);
    if (brick != 0) {
        int brickIndex = reinterpret_cast<size_t>(brick) - 1;
