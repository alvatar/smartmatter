...
void main() {
    ...
    vec4 curColor = textureLookup2Dscreen(colorTex_, textureParameters_, fragCoord);

    // initialize Gaussian filter
    halfKernelDim_ = int(2.5 * sigma_);
    float norm = computeGaussKernel();
    // apply Gaussian only to selected channels
    FragData0 = blurChannels*applyColorGaussFilter(fragCoord, norm, dir_) + nblurChannels*curColor;
    float depth;
