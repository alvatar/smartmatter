...
#include "voreen/modules/dicom/dicomvolumewriter.h"
...
const std::string DicomVolumeWriter::loggerCat_("voreen.modules.dicom.DicomVolumeWriter");
...
VolumeWriter* DicomVolumeWriter::create(ProgressBar* /*progress*/) const {
    return new DicomVolumeWriter();
}

void DicomVolumeWriter::write(const std::string& fileName, VolumeHandle* volumeHandle)
    throw (tgt::IOException) {

    if (!volumeHandle || !volumeHandle->getVolume()) {
        LERROR("Empty volume handle passed");
        return;
    }

    DicomVolumeDescription desc;
    DicomVolumeWriter::writeDicomFiles(desc, volumeHandle->getVolume(), fileName);
}
