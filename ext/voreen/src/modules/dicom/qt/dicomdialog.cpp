...
void ServerConfigDialog::enableSSL(int enable) {
...
void ServerConfigDialog::editConfig(ServerConfig* conf) {
    currentConf_ = conf;
...
void ServerDialog::enableOpenButton() {
    openButton_->setEnabled(true);
}

void ServerDialog::selectOutputDir() {
...
void ServerDialog::cancelDownload() {
    voreen::DicomMoveSCU::cancelMove();
}
...
void ServerDialog::downloadFinished() {
    t_->stop();
...
std::string ServerDialog::getURL() {
    if (listWidget_->getSelectedSeriesInstanceUID().empty())
        return "";
    else
...
void SeriesListWidget::setSeries(std::vector<voreen::DicomSeriesInfo>& list) {
//     table->clear();
    table->setSortingEnabled(false);  // disable sorting temporarly for inserting new items
    std::vector<voreen::DicomSeriesInfo>::iterator theIterator;
    int c = 0;
...
int SeriesListWidget::getSelectedNumImages() {
...
std::string DicomDirDialog::getFilename() {
    if (listWidget_->getSelectedSeriesInstanceUID().empty())
        return filename_;
    else
