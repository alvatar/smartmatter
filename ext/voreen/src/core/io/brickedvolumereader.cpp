...
VolumeCollection* BrickedVolumeReader::read(const std::string & filename)
    throw (tgt::CorruptedFileException, tgt::IOException, std::bad_alloc) {

    return readSlices(filename,0,0);
}

VolumeCollection* BrickedVolumeReader::readSlices(const std::string & filename, size_t , size_t)
    throw (tgt::CorruptedFileException, tgt::IOException, std::bad_alloc)
{
    openFile(filename);
    ...
    brickedHandle->setModality(Modality::MODALITY_BRICKED_VOLUME);
    brickedHandle->setOrigin(VolumeOrigin(filename));
    volumeCollection->add(brickedHandle);
