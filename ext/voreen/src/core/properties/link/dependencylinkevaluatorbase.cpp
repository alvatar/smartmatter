...
DependencyLinkEvaluatorBase::DependencyLinkEvaluatorBase()
    : LinkEvaluatorBase()
    , historyLength_(-1)
{}

void DependencyLinkEvaluatorBase::eval(Property* src, Property* dst) throw (VoreenException) {
...
std::string DependencyLinkEvaluatorBase::serializeProperty(Property* p) const {
    XmlSerializer s;
...
void DependencyLinkEvaluatorBase::deserializeProperty(Property* p, std::string s) const {
    XmlDeserializer d;
...
std::string DependencyLinkEvaluatorBase::name() const {
    return "DependancyLink";
}

void DependencyLinkEvaluatorBase::setHistoryLength(int newLength) {
    historyLength_ = newLength;
}

int DependencyLinkEvaluatorBase::getHistoryLength() const {
    return historyLength_;
}

void DependencyLinkEvaluatorBase::setKeyValue(std::string key, std::string value) {
    tgtAssert(keys_.size() == values_.size(), "keys and values have different number of entries");
...
void DependencyLinkEvaluatorBase::removeFrontItemFromMap() {
    tgtAssert(keys_.size() == values_.size(), "keys and values have different number of entries");
...
void DependencyLinkEvaluatorBase::clearDependencyMap() {
    keys_.clear();
    values_.clear();
}

void DependencyLinkEvaluatorBase::serialize(XmlSerializer& s) const {
    tgtAssert(keys_.size() == values_.size(), "keys and values have different number of entries");
    s.serialize("keys", keys_);
    s.serialize("values", values_);
}

void DependencyLinkEvaluatorBase::deserialize(XmlDeserializer& d) {
    d.deserialize("keys", keys_);
    d.deserialize("values", values_);
    tgtAssert(keys_.size() == values_.size(), "keys and values have different number of entries");
}

void DependencyLinkEvaluatorBase::propertiesChanged(Property* src, Property* dst) {
